"""MC-dropout uncertainty for one ABMIL + Cox model, in two phases.

This is the MC-dropout-only counterpart to
cross_validate_abmil_cox_per_uq_method.py (which runs all three UQ methods on
shared backbones). Here we do one method, end to end:

    Phase 1 -- 5-fold cross-validation
        Fold the cohort once (make_cv_folds), and for every fold train a fresh
        ABMIL model and run MC-dropout on that fold's held-out patients. Each
        patient is scored exactly once across the folds, so the per-fold and
        pooled C-index / selective-AUC estimate how MC-dropout generalises
        without spending a fixed test set.

    Phase 2 -- frozen-split refit
        Train one model on the frozen train/val split stored in the CSV, then
        run MC-dropout on that CSV's held-out test set. This is the single
        reportable model + its held-out metrics (C-index + patient-level
        bootstrap CI, selective-prediction AUC, mean predictive uncertainty).

Architecture and optimisation knobs match the arch-screen winner
(baseline_input_norm, pure Cox), so the numbers line up with the sibling
scripts. Outputs (metric CSVs + per-slide predictions) are written to RESULT_DIR.

Example:
    python scripts/training/mc_dropout_abmil_cox.py
"""
...
RESULT_DIR = PROJECT_ROOT / "results" / "mc_dropout_abmil_cox_tcga_gbmlgg"
...
MODEL_CONFIG = dict(
    ...
    input_norm=True,
    gated=True,
    dropout=0.25,  # active at inference -> the source of MC-dropout uncertainty
)
...
def fit_model(train_loader, val_loader, device, loss_fn, seed):
    """Train one ABMIL model (best-epoch weights kept in memory).
...
def save_model(model, optimizer, history, path):
    """Checkpoint a trained model so src.eval.load_model can rebuild and score it.
...
def score_uq_frame(uq_df):
    """Patient-level metrics for one MC-dropout frame: C-index, selective AUC, mean std."""
...
def run_cross_validation(table, device, loss_fn):
    """MC-dropout under shared K-fold CV.
    ...
    for fold_index, fold_table in folds:
        ...
        uq_df = mc_dropout_predict(
            model, test_loader, device, n_samples=MC_DROPOUT_SAMPLES
        )
...
def run_frozen_split(table, device, loss_fn):
    """Train on the CSV's frozen train/val split; MC-dropout on its held-out test.

    Returns (metrics, test_predictions, history). metrics carries both the
    deterministic (dropout-off) C-index + bootstrap CI and the MC-dropout
    mean-risk C-index + selective-prediction / uncertainty numbers on the same
    test patients; history is the train() run for the learning curves.
    """
    ...
    model, history, optimizer = fit_model(train_loader, val_loader, device, loss_fn, SEED)
    save_model(model, optimizer, history, CHECKPOINT_DIR / "frozen_split.pt")

    # Deterministic (dropout-off) baseline: C-index + patient-level bootstrap CI.
    deterministic = evaluate(model, test_loader, device, n_boot=N_BOOT, seed=SEED)

    # MC-dropout: mean risk + predictive uncertainty on the same test patients.
    uq_df = mc_dropout_predict(model, test_loader, device, n_samples=MC_DROPOUT_SAMPLES)
    patients = to_patient_frame(uq_df)
    mc_point, mc_lo, mc_hi, _ = bootstrap_cindex(
    ...
    metrics = {
        ...
        "mc_ci_low": mc_lo,
        "mc_ci_high": mc_hi,
        "mc_selective_auc": selective_auc(patients),
        "mc_mean_risk_std": float(patients["risk_std"].mean()),
    }
...
def main():
    ...
    print(f"\n{'Model':<14}{display_model.__class__.__name__}  "
          f"({n_params:,} params, {head_kind} head)")
    ...
    print(f"{'MC-Dropout':<14}samples={MC_DROPOUT_SAMPLES}  bootstrap={N_BOOT}")
    ...
    print(
        f"{'MC-dropout':<14}C-index {test_metrics['mc_cindex']:.4f}   "
        f"95% CI [{test_metrics['mc_ci_low']:.4f}, {test_metrics['mc_ci_high']:.4f}]"
    )
    print(
        f"{'':<14}sel-AUC {test_metrics['mc_selective_auc']:.4f}   "
        f"risk-std {test_metrics['mc_mean_risk_std']:.4f}"
    )
