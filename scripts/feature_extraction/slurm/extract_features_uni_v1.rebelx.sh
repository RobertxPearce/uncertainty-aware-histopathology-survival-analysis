#!/bin/bash
# extract_features_uni_v1.rebelx.sh
#
# SLURM wrapper for the RebelX GPU cluster. Paired with the cluster-agnostic
# worker scripts/feature_extraction/extract_features_trident_uni_v1.py.
#
# Runs TRIDENT UNI v1 feature extraction over every .svs under data/raw/slides
# on the A30 GPUs. Outputs follow the repo layout (cohort defaults to TCGA_LUAD):
#   data/processed/features/uni_v1/<cohort>/ -> one UNI v1 embedding .h5 per slide
...
#
#SBATCH --job-name=trident_feat_v1
...
#SBATCH --output=logs/trident_v1_%A_%a.out
...
# Run
srun --unbuffered python scripts/feature_extraction/extract_features_trident_uni_v1.py
