"""
Extract TRIDENT UNI v2 (UNI2-h) patch features for every slide in a cohort.

Per slide the TRIDENT pipeline runs: tissue segmentation -> patch coords ->
UNI v2 patch features, writing one flat .h5 feature bag per slide.

Example Run on a GPU node:
    python scripts/feature_extraction/extract_features_trident_uni_v2.py \
...
# 20x, segmentation at 10x (TRIDENT resizes each patch to 224px internally).
# Only the encoder and output embedding dim differ (uni_v2 -> 1536, uni_v1 -> 1024).
TARGET_MAG = 20
PATCH_SIZE = 256
SEG_MAG    = 10
